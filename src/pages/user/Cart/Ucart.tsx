...
const Ucart = () => {
            ...
            if (getCart.data.status === "Success") {
              ...
              if (getLastCheckout.data.status === "Success") {
                ...
                if (lastCheckout.length) {
                  ...
                  cart?.forEach((ct: IPropsProductList) => {
                    var lcp = lastCheckout.filter((lco: IPropsProductList) => (lco._id === ct._id))[0]
                    if (lcp) {
  ...
  const selectProduct = (id: string) => {
    var orderData = orderCartData.map((od: IPropsProductList) => {
      if (od._id === id) {
    ...
    var data = orderData.filter((od: IPropsProductList) => od.isSelect)
  ...
  const checkOut = async () => {
    ...
    if (orderDataClone.length) {
      var filterUserQuantity = orderDataClone.map((ocd: IPropsProductList) => {
        ocd.quantityAndTypeAndPrice = ocd.quantityAndTypeAndPrice.filter((qtp: IPropsQTP) => (qtp.userQuantity as number) > 0);
        return ocd;
      });
      var selectedWithEmptyQuantity = filterUserQuantity.filter((ocd: IPropsProductList) => ocd.quantityAndTypeAndPrice.length === 0);
      if (selectedWithEmptyQuantity.length) {
        ...
        var finalOrderListData = filterUserQuantity.filter((ocd: IPropsProductList) => ocd.quantityAndTypeAndPrice.length > 0);
  ...
  const getTotalPrice = () => {
    var finalPrice = orderCartData.reduce((total: number, ocd: IPropsProductList) => {
