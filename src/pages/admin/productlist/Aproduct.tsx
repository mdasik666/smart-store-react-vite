import { Alert, Button, Divider, IconButton, FormControl, InputLabel, Select, MenuItem, Modal, Paper, Snackbar, Stack, TextField, Typography, FormHelperText, CircularProgress, AlertColor, Avatar, Tooltip } from "@mui/material";
...
import ConfirmDialog from "../../../components/admindrawer/Dialog";
...
const Aproduct = () => {
    ...
    const { register, handleSubmit, formState: { errors, isValid, dirtyFields, touchedFields }, reset, setValue, unregister } = useForm({
        mode: "onChange",
    ...
    return (
        <>
                                                    ...
                                                    return (
                                                        <TableRow hover role="checkbox" tabIndex={-1} key={prodList._id}>
                                                            {productKey?.map((plKey: any, i: number) => {
                                                                ...
                                                                return (
                                                                    String(value)?.startsWith("data:image") ? <TableCell key={i} align={"center"}><Avatar src={value} /></TableCell> : <TableCell key={i} align={"center"}>{value}</TableCell>
                                                                );
                                                            })}
            ...
            <ConfirmDialog open={openDialog} onClose={handleCloseDialog} onConfirm={handleDeleteConfirmAction} />
        </>
    )
}
